...
import { login, signup } from "../../storage/storage";
  ...
  const Login = () => {
    ...
    const handleLogin = async () => {
        if(email.length===0){
          ...
          if(res === "There is no User yet please signup first"){
             ...
             Alert.alert("Message",res,[{text:'Ok',onPress:()=>navigation.navigate("GameStack")}]);
          }
        }
    ...
    useEffect(()=>{
      async function autoSignup(){
        signup(shortid.generate(),"admin","admin@gmail.com","admin1234")
      }
      autoSignup()
    },[])
